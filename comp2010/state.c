...
STATE(init_back,  9000, AUTO_STRAIGHT(30, -127), auto_straight_init, auto_straight_loop, init_dump1, init_dump1, auto_straight_isdone)
...
#if 0
/* Deploy the arm from its starting ("up") position. */
STATE(start,       1,    AUTO_WAIT(),             auto_none_init,     auto_none_loop,     deploy,      deploy, auto_none_isdone)
STATE(deploy,      150,  AUTO_DEPLOY(),           auto_deploy_init,   auto_deploy_loop,   init_ram,    done,   auto_none_isdone)

/* Dump the preloaded balls. */
STATE(init_ram,    9000, AUTO_STRAIGHT(0, 127),   auto_straight_init, auto_straight_loop, init_arm,    init_arm,   auto_straight_isdone)
STATE(init_arm,    2000, AUTO_ARM(0, -127),       auto_arm_init,      auto_arm_loop,      init_dump1,  done,       auto_arm_isdone)
STATE(init_dump1,  2000, AUTO_RAMP(100, 127),     auto_ramp_init,     auto_ramp_loop,     init_dump2,  init_dump2, auto_ramp_isdone) 
STATE(init_dump2,  5000, AUTO_WAIT(),             auto_none_init,     auto_none_loop,     init_dump3,  init_dump3, auto_none_isdone)
STATE(init_dump3,  2000, AUTO_RAMP(0, -127),      auto_ramp_init,     auto_ramp_loop,     coll_drive1, done,       auto_ramp_isdone)

/* Pickup the orange footballs. */
...
STATE(coll_arm1,   2000, AUTO_PICKUP(127),        auto_pickup_init,   auto_pickup_loop,   coll_arm2,   done,   auto_pickup_isdone)
