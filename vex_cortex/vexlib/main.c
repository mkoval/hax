...
int main(void)
{
  ...
  while (1)
  {
    ...
    {
      if ((sendit == 0) && (SendDataToUart)) Debug_Send();
      sendit = (sendit + 1) % 6;

      if ((systemStateFlags & 0x20) == 0x20) 
        printf("rcvr %x tmo1 %d tmo2 %d\r\n",pwmStatusFlags,rx1Timeout,rx2Timeout);
      updateMaster = 0;
      ...
      if (masterPtr->Sync == 0xC917)      //Valid Sync?
      {
        ...
        {
          printf("Waiting for Valid Data from Master st %x\r\n",masterPtr->State);
      ...
      if (((systemStateFlags & 1) == 1) && ((masterPtr->packetNum % 5) == 0)) //Display SPI data at a slower rate
      {
        ...
        {
          b1 = masterPtr->backupBatteryVoltage * 0.0591;
          m2 = masterPtr->mainBatteryVoltage * 0.0591;
      #if 1
            for (j=0;j<MAX_MOTORS;j++) slavePtr->Analog[j] = ADCConvertedValue[j] >> 2;
            printf("%x:",masterPtr->SystemFlags);
            for (j=0;j<4;j++) printf("%02x ",masterPtr->joystick1[j]);
            //printf(", %x %x %x %x %x %x %x %x %x %x",mot1Dir,mot2Dir,mot3Dir,mot4Dir,mot5Dir,mot6Dir,mot7Dir,mot8Dir,mot9Dir,mot10Dir);
            printf(": %x %x :",ConfigurationMode,analogMask);
            for (j=0;j<MAX_MOTORS;j++) printf("%02x ",slavePtr->Analog[j]);
      #else
          if (crystalActive)
          {
            printf("M %x S %x:",masterPtr->SystemFlags,slavePtr->SystemFlags);
            for (j=0;j<MAX_MOTORS;j++) printf("%02x ",slavePtr->Motor[j]);
          }
          else
          {
            printf("%x:",masterPtr->SystemFlags);
            for (j=0;j<9;j++) printf("%02x ",masterPtr->joystick1[j]);
            printf(",");
            for (j=0;j<9;j++) printf("%02x ",masterPtr->joystick2[j]);
          }
      #endif
        ...
        {
            for (j=1;j<SPI_PACKET_SIZE-1;j++) printf("%02x",SPI1_Buffer_Rx[j]);
            printf(" %02x %02x",masterPtr->version,masterPtr->packetNum);
        ...
        else
        {
          ...
          {
            for (j=0;j<8;j++) printf("%04x",SPI1_Buffer_Rx[j]);
            slavePtr->DigitalByte2 = 2;
          }
          else
          {
            for (j=0;j<8;j++) printf("%04x",SPI1_Buffer_Rx[j+8]);
...
void Handle_Operator_Control()
{
  ...
  for (i=0;i<MAX_MOTORS;i++) //Update Motors
  {
    ...
    {
      if (i == 6) ptr = &masterPtr->joystick2[0];
  ...
  if (crystalActive == 0)
  {
    switch (ConfigurationMode)
    {
      ...
      case 2: //Single Driver Arcade
        One_Stick(1,JOY1_CHAN2,JOY1_CHAN1,&motorR,&motorL);
...
void WaitTillMasterIsReadyForSpiData(void)
{
  ...
  while (masterNotReady)  //Loop Here until Master is ready to handle SPI data
  {
    ...
    {
      delay = 0;
      if (masterNotReady) printf("Waiting ...\r\n");
    }
  } 
  printf("Wait Done ...\r\n");     
}
...
void Handle_Data_From_Crystal(u8 receiverPort)
{
  ...
  start = 1;
  if ((systemStateFlags & 8) == 8) start = 0;
  for (i=start;i<7;i++)
  {
    rxPtr[i] = pwmPtr[i];
    ...
    rxPtr[i] = (u16) fPWM;
    if (rxPtr[i] > 255) rxPtr[i] = 255;
    if ((rxPtr[i] >= 123) && (rxPtr[i] <= 131)) rxPtr[i] = 127; 
